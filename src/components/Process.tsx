    ...
    <section id="process" className="relative py-28 md:py-40">
      <div className="mx-auto max-w-[1500px] px-5 md:px-10">
        ...
        <div className="mt-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <Reveal><h2 className="font-display max-w-2xl text-5xl font-black leading-[0.95] md:text-7xl">DIVE. <span className="text-outline">DRIFT.</span><br /><span className="shimmer-text italic">TAKE IT HOME.</span></h2></Reveal>
          ...
          <div className="space-y-6">
            {STEPS.map((s, i) => (
              <Reveal key={s.n} delay={i * 0.08}>
                <article data-hover className="group relative grid gap-6 rounded-sm border border-foam/10 bg-deep/30 p-7 transition-all duration-500 hover:border-aqua/40 hover:bg-deep/60 hover:shadow-[0_0_60px_rgba(70,212,195,0.08)] md:grid-cols-12 md:items-center md:p-10 md:pl-24">
                  <span className="font-display text-outline-faint absolute right-6 top-4 text-7xl font-black transition-all duration-700 group-hover:text-transparent md:text-8xl" aria-hidden>{s.n}</span>
                  ...
                  <div className="md:col-span-4"><p className="text-[11px] uppercase tracking-[0.4em] text-aqua">Step {s.n}</p><h3 className="font-display mt-2 text-3xl font-light text-foam md:text-4xl">{s.title}</h3></div>
