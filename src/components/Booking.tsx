  ...
  return (
    <section id="book" className="relative overflow-hidden py-28 md:py-40">
      ...
      <div className="relative mx-auto max-w-[1500px] px-5 md:px-10">
        ...
        <Reveal><h2 className="font-display mt-8 max-w-4xl text-5xl font-black leading-[0.95] md:text-8xl">SEE YOU <span className="text-outline">DOWN</span><br /><span className="shimmer-text italic">THERE.</span></h2></Reveal>
        <div className="mt-16 grid gap-8 lg:grid-cols-12">
            ...
            <div className="relative h-full overflow-hidden rounded-sm">
              ...
              <div className="relative flex h-full min-h-[520px] flex-col justify-between p-8 md:p-10">
                <div>
                  ...
                  <p className="font-display mt-4 text-3xl font-light leading-snug text-foam">One message. I handle the dive shop, the boat, the light — you just show up and breathe.</p>
                </div>
          ...
          <Reveal delay={0.12} className="lg:col-span-7">
            <div className="glass relative h-full overflow-hidden rounded-sm p-8 md:p-12">
              ...
              <AnimatePresence mode="wait">
                {status === 'done' ? (
                  <motion.div key="done" initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} className="flex h-full min-h-[480px] flex-col items-center justify-center text-center">
                    ...
                    <h3 className="font-display mt-8 text-4xl font-light">You&apos;re on the manifest.</h3>
                  ...
                  <motion.form key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, y: -12 }} onSubmit={submit} noValidate className="relative space-y-6">
                    <div className="grid gap-6 sm:grid-cols-2">
                      <div className={inputWrap}><label className={labelCls} htmlFor="bk-name">Your name</label><input id="bk-name" className="field" placeholder="Jacques Cousteau" value={form.name} onChange={(e) => set('name', e.target.value)} />{errors.name && <Err msg={errors.name} />}</div>
                      <div className={inputWrap}><label className={labelCls} htmlFor="bk-email">Email</label><input id="bk-email" type="email" className="field" placeholder="you@somewhere.blue" value={form.email} onChange={(e) => set('email', e.target.value)} />{errors.email && <Err msg={errors.email} />}</div>
                    </div>
                    <div className="grid gap-6 sm:grid-cols-3">
                      ...
                      <div className={inputWrap}><label className={labelCls} htmlFor="bk-pkg"><Camera size={13} /> Package</label><select id="bk-pkg" className="field" value={form.package} onChange={(e) => set('package', e.target.value)}><option value="">Select…</option>{pkgOptions.map((p) => <option key={p.id} value={p.name}>{p.name} — ${p.price}</option>)}</select>{errors.package && <Err msg={errors.package} />}</div>
                    </div>
